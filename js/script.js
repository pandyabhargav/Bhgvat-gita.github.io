...
// var slockLength;
  ...
  // Call the fetchChapters function
  const update = (newChapterNumber) => {
    fetchChapters1(newChapterNumber);
  }
  
  for (let i = 1; i <= 18; i++) {
    document.querySelector(`.bg-col-btn${i}`).addEventListener('click', () => {
      update(i);
    });
  }  
  ...
  document.addEventListener("DOMContentLoaded", function() {
    ...
    document.addEventListener("click", function() {
        // Check if the audio is paused (not already playing)
        if (audio.paused) {
    ...
    // Set the loop property to true so the audio loops
    audio.loop = true;
});
